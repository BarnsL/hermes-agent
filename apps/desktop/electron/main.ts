  ...
  const contextMenu = Menu.buildFromTemplate([
    { label: 'Open Hermes', click: showOrCreate },
    {
      ...
      click: () => {
        ...
        // re-runs startHermes() and boots a fresh backend. invalidate() (not
        // getProcess()) so the connection state forgets the dying child and
        // the reload spawns fresh instead of reattaching.
        stopBackendChild(backendConnectionState.invalidate())
        stopAllPoolBackends()
        setTimeout(() => {
          if (!mainWindow || mainWindow.isDestroyed()) {
            createWindow()
