 ...
 *
 * Native HTML5 drag-and-drop hands the gesture to the OS after `dragstart`.
 * On some setups (remote-desktop and virtual input drivers are the known
 * offenders) the OS drag loop aborts the moment it starts: `dragstart` fires,
 * then no dragover/dragend/drop ever arrives, and no web-side code can revive
 * it. This module implements the same interaction with raw pointer events,
 * which never leave the renderer.
 *
 * Coexistence with native DnD is self-arbitrating:
 * - Healthy machine: the native drag loop engages a few px into the gesture
 *   and the OS STOPS delivering pointermove — this tracker never reaches its
 *   threshold and stays dormant; the HTML5 drop zones handle everything.
 * - Broken machine: the native loop dies immediately, pointer events keep
 *   flowing (the button is still held), the tracker crosses its threshold and
 *   takes over: DOM ghost, zone hit-testing via the registry below, the same
 *   isOver highlight, the same drop callback.
 *
...
function makeGhost(title: string): HTMLDivElement {
  ...
  // Language-neutral fallback for the rare untitled session.
 ...
 * pointer travels past the threshold while the native drag loop is dead, runs
 * the fallback drag. No-ops for non-primary buttons, modified clicks
 * (shift-pin / meta-new-window), touch, and reorder-handle presses.
 */
export function startSessionPointerTracking(event: React.PointerEvent, payload: SessionDragPayload) {
  ...
  const onMove = (e: PointerEvent) => {
    ...
    // The environments this fallback exists for are event-lossy: if the
    // pointerup itself got swallowed, the button state is the ground truth —
    // never keep dragging with no button held.
    if (e.buttons === 0) {
      endDrag(false)
