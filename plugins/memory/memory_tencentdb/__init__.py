    ...
    for candidate in searched:
        try:
            if candidate.is_file():
                ...
                # ─── RECURRING ISSUE R-3: BROKEN ON WINDOWS — NO `sh` ─────────
                # SYMPTOM: 1,662 "Failed to start memory-tencentdb Gateway:
                #   [WinError 2] The system cannot find the file specified"
                #   in errors.log on 2026-07-08 (13:17→21:29), right after a
                #   session re-installed this plugin. The supervisor Popen()s the
                #   string returned here; on Windows there is NO `sh` on PATH, so
                #   the very first token (`sh`) fails with WinError 2, forever, on
                #   a retry loop (~1 failure/7s).
                # ROOT CAUSE: this returns a hardcoded `sh -c ...` command. That
                #   is Unix-only. This plugin, as written, CANNOT run on Windows.
                # WHY IT RECURS: an earlier fix set `memory.provider: none`
                #   (sonic-hermes.md), but a later "install TencentDB" session
                #   undid it and re-enabled the plugin — with no guard to refuse
                #   an integration that is known-broken on this OS.
                # HARDENING (see RECURRING-ISSUES-AND-GAMEPLAN-2026-07-09.md G-5):
                #   On os.name == "nt", either emit a Windows-native launcher
                #   (e.g. `cmd /c "cd /d <root> && pnpm exec tsx ..."`) instead of
                #   `sh -c`, OR raise a clear "not supported on Windows" error at
                #   enable-time so this can never re-enter the WinError-2 loop.
                # ──────────────────────────────────────────────────────────────
                ...
                # NOTE: `sh -c` below is the exact token that fails on Windows.
